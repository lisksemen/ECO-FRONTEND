...
function PollutionDeleteButton({ pollution, onDelete }) {
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(`http://localhost:8080/api/v1/pollutions/${pollution.id}`);
            onDelete();
            setIsConfirmationVisible(false); // Hide the confirmation dialog
        } catch (error) {
            console.error('Error deleting pollution data:', error);
            setError('An error occurred while deleting the pollution data.'); // Set error message
        }
    };
...
export default PollutionDeleteButton;
